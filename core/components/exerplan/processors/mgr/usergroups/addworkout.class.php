...
class UsergroupsAddWorkoutsProcessor extends modObjectUpdateProcessor {

    public $classKey = 'Workouts';
    public $languageTopics = array('exerplan:cmp');
    public $objectType = 'exerplan.UsergroupsAddWorkouts';

    public function beforeSave() {
        $props = $this->getProperties();
        $check = $this->modx->getObject('UsergroupsWorkouts', array(
            'usergroup_id' => $props['usergroup_id'],
            'workout_id' => $props['id']
        ));
        if ($check) {
            $this->addFieldError('name', $this->modx->lexicon('exerplan.UsergroupsWorkouts_err_ae'));
        }
    ...
    public function process() {
        ...
        $props = $this->getProperties();
        $newObject = $this->modx->newObject('UsergroupsWorkouts');
        $newObject->set('usergroup_id', $props['usergroup_id']);
        $newObject->set('workout_id', $props['id']);

        $newUsergroupsWorkouts = array($newObject);
        $this->object->addMany($newUsergroupsWorkouts);
...
return 'UsergroupsAddWorkoutsProcessor';