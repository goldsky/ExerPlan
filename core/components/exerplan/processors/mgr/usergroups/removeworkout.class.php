...
class UsergroupsWorkoutsRemoveProcessor extends modObjectRemoveProcessor {

    public $classKey = 'UsergroupsWorkouts';
    public $languageTopics = array('exerplan:cmp');
    public $objectType = 'exerplan.UsergroupsWorkoutsRemove';

    public function initialize() {
        $workoutKey = $this->getProperty('workout_id', false);
        if (empty($workoutKey))
            return $this->modx->lexicon($this->objectType . '_err_ns');
        $usergroupKey = $this->getProperty('usergroup_id', false);
        if (empty($usergroupKey))
            return $this->modx->lexicon($this->objectType . '_err_ns');

        $this->object = $this->modx->getObject($this->classKey, array(
            'usergroup_id' => $usergroupKey,
...
return 'UsergroupsWorkoutsRemoveProcessor';