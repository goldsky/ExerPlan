...
class WorkoutsCreateProcessor extends modObjectCreateProcessor {

    public $classKey = 'Workouts';
    public $languageTopics = array('exerplan:cmp');
    public $objectType = 'exerplan.WorkoutsCreate';

    public function beforeSave() {
        $name = $this->getProperty('name');

        if (empty($name)) {
            $this->addFieldError('name', $this->modx->lexicon('exerplan.name_err_ns_name'));
        } else if ($this->doesAlreadyExist(array('name' => $name))) {
            $this->addFieldError('name', $this->modx->lexicon('exerplan.name_err_ae'));
        }
...
return 'WorkoutsCreateProcessor';