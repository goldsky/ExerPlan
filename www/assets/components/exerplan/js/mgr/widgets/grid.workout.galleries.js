ExerPlan.grid.WorkoutGalleries = function(config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        id: 'exerplan-grid-workout-galleries',
        url: ExerPlan.config.connectorUrl,
        baseParams: {
            action: 'mgr/galleries/getList',
            workout_id: config.node.attributes.wid
        },
        fields: [
    ...
    ExerPlan.grid.WorkoutGalleries.superclass.constructor.call(this, config);
};

Ext.extend(ExerPlan.grid.WorkoutGalleries, MODx.grid.Grid, {
    getMenu: function() {
        return [{
        ...
        if (!this.addUrlWindow) {
            this.addUrlWindow = MODx.load({
                ...
                baseParams: {
                    ...
                    workout_id: this.node.attributes.wid
                },
                listeners: {
                    'success': {
                        fn: this.refresh,
...
Ext.reg('exerplan-grid-workout-galleries', ExerPlan.grid.WorkoutGalleries);
